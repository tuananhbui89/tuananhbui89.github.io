...
% --- 必须补充的宏包库 ---
\usepackage{fancyvrb} % 支持 Verbatim 环境
\usepackage[most]{tcolorbox}
\tcbuselibrary{listings, skins, breakable} % 必须加 breakable，否则长 Prompt 会报错
\usepackage{fvextra} % 必须在 fancyvrb 之后，支持自动换行
\fvset{breaklines=true, breakanywhere=true} % 全局开启 Verbatim 自动换行
\usepackage{listings}
...
\usepackage{fancyvrb}
\usepackage{fvextra} % fancyvrb 개선 패키지
...
% , and demonstrate through experiments that {\our{}} consistently achieves higher performance than existing strong baselines;
...
% However, the effectiveness of the LLM-as-a-Judge framework may be limited by inherent biases in LLMs~\citep{wang_large_2023}. To mitigate these issues, 
...
\begin{algorithmic}[1]
    ...
    \FOR{each $(q, a) \in \mathcal{D}$}
        ...
        \IF{ $s\geq \varepsilon$}
            \STATE \textbf{continue} %\COMMENT{\begingroup\scriptsize\texttt{If success, no judging needed, skipping to next sample}\endgroup}
        \ELSE
            \STATE $(r_i)_{i=1}^M \leftarrow \text{Judge}(W, q, a, \{a_i'\}_{i=1}^M)$ %\COMMENT{\begingroup\scriptsize\texttt{Call Judge to rank blocks by responsibility}\endgroup}
            
            \STATE $\mathcal{R}_{\text{ranks}} \gets \textsc{Append}(\mathcal{R}_{\text{ranks}}, (r_i)_{i=1}^M)$ %\COMMENT{\begingroup\scriptsize \texttt{Append block-wise judge ranking}\endgroup}
            \STATE $B_{\text{rw}} \leftarrow \texttt{RoundWorst}((r_i)_{i=1}^M, W)$ %\COMMENT{\begingroup\scriptsize \texttt{Get the most problematic block in this round}\endgroup}
            \STATE $\mathcal{L}_{B_{\text{rw}}} \gets \textsc{Append}(\mathcal{L}_{B_{\text{rw}}}, (q, a, \{a_i'\}_{i=1}^M))$ %\COMMENT{\begingroup\scriptsize \texttt{Append failure execution context to that block’s log }\endgroup}
        \ENDIF
    \ENDFOR
    \STATE $B_{\text{sel}} \leftarrow \texttt{OverallWorst}(\mathcal{R}_{\text{ranks}}, W)$ %\COMMENT{\begingroup\scriptsize\texttt{Aggregate across failures to pick the globally weakest block}\endgroup}
    \STATE $P_W \leftarrow \texttt{CalPerformance}(\mathcal{P}_{\text{scores}})$ %\COMMENT{\begingroup\scriptsize\texttt{Compute overall performance on the dataset}\endgroup}
...
% The experiments are designed to evaluate whether block-level judging introduces effective credit assignment under fixed model capacity by converting coarse end-to-end guidance into fine-grained, block-level signals that enable targeted workflow optimization.
...
% \paragraph{Extension to Complex Benchmarks. }
...
\begin{axis}[
    ...
    height=5.5cm, % 稍微增高一点
    % --- 修改 1：改用百分比范围，并收紧上限 ---
    ymin=35, ymax=48, % 最高点约47.5%，设为52比较紧凑
    ylabel={Accuracy (\%)}, % 修改标签
    % -------------------------------------
    xmin=0.5, xmax=2.5,
    xtick={1,2},
    xticklabels={AFlow, JudgeFlow},
    bar width=25pt, % 稍微加宽柱子
    ylabel style={yshift=-5pt},
    ymajorgrids=true,
    grid style={dashed, gray!30}, % 网格线淡一点，不喧宾夺主
    % --- 修改 2：添加箭头 ---
    axis x line=bottom, % 去掉星号 *
    axis y line=left,   % 去掉星号 *
    axis line style={->, >=stealth, thick}, % 设置箭头样式为 stealth，并加粗坐标轴
    % -----------------------
]
...
coordinates {(1, 42.00) +- (0, 5.00)}; % 数据 x 100

% --- JudgeFlow (改为百分比数据) ---
\addplot[
    ybar,
    fill=red!40, % 稍微加深一点蓝色让对比更明显
    draw=black,
    error bars/.cd,
        y dir=both,
        y explicit,
...
coordinates {(2, 44.67) +- (0, 2.78)}; % 数据 x 100

% --- Value labels (改为百分比格式) ---
\addplot[
    only marks,
    mark=none,
    nodes near coords,
    point meta=explicit symbolic,
    nodes near coords style={anchor=south, yshift=0pt, font=\small\bfseries}, % 加粗标签
]
coordinates {
    (1, 42.00) [42.00] % 加上 % 号
    (2, 44.67) [44.67] % 加上 % 号，保留一位小数即可，看着更整洁
};
...
\paragraph{Ablation}
% \cref{fig:ab2} compares the training curve and testing curve of the highest performance found between \our{} and AFlow. \our{} exhibits clear performance gains within the first five optimization iterations, with both the training and testing curves showing rapid improvements. Beyond this early stage, \our{} continues to achieve gains, ultimately converging to higher accuracy. In contrast, AFlow remains stagnant across most iterations and only shows noticeable improvements in the later stage, and its final training and testing performance remain consistently lower than those of \our{}.
...
\bottomrule
\end{tabular}
% \captionof{table}{Testing performance using different LLMs on MBPP dataset.}
% \label{tab:ablation}
\end{table}
