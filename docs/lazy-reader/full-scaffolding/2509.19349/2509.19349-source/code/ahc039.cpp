...
void calculate_score_delta_for_rectangle(KDNode* node, int r_min_x, int r_max_x, int r_min_y, int r_max_y, 
    ...
    while (!stack.empty()) {
        ...
        // Disjoint?
        if (current_node->max_x < r_min_x || current_node->min_x > r_max_x || current_node->max_y < r_min_y || current_node->min_y > r_max_y) {
            continue;
        }
        // Fully inside?
        if (r_min_x <= current_node->min_x && current_node->max_x <= r_max_x && r_min_y <= current_node->min_y && current_node->max_y <= r_max_y) {
...
void simulated_annealing_main() {
    ...
    bool current_poly_initial_valid = is_polygon_structurally_sound(current_state.poly, current_state.perimeter_cache) &&
    ...
    if (!current_poly_initial_valid) { 
         ...
         calculate_score_from_scratch(current_state.poly, current_state.m_count, current_state.s_count);
         current_state.perimeter_cache = calculate_perimeter(current_state.poly); // Update perimeter for default
    }
    ...
    while (global_timer.elapsed() < ACTUAL_TIME_LIMIT_SECONDS) {
        ...
        if (move_type_roll < p_targeted && candidate_state.poly.size() >= 4) { // Targeted Edge Move
            ...
            int fish_idx = target_mackerel ? rng.next_int(n_fish_half) : n_fish_half + rng.next_int(n_fish_half);
